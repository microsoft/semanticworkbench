...
if TYPE_CHECKING:
    from .skill import BoundSkill, Skill
...
class RunContext:
    ...
    def __init__(
        ...
        get_state: Callable[[], Awaitable[dict]],
        set_state: Callable[[dict], Awaitable[None]],
        ...
        self._bound_skills: dict[str, "BoundSkill"] = {name: skill.bind_context(self) for name, skill in skills.items()}

    def __getattr__(self, name: str) -> "BoundSkill":
        """Makes skills available as attributes, e.g. context.posix"""
        if name in self._bound_skills:
            return self._bound_skills[name]
        raise AttributeError(f"No skill named '{name}'")
