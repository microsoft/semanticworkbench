...
class DocumentAgent:
    ...
    def _save_state(self, state: State, context: ConversationContext) -> None:
        if state is None:
            logger.error("Document Agent: state to save is None. Cannot write to storage.")
            return
        self._state = state
        write_document_agent_conversation_state(context, state.model_dump())

    def _load_state(self, context: ConversationContext) -> State | None:
        state_dict = read_document_agent_conversation_state(context)
        if state_dict is not None:
            self._state = State(**state_dict)
        else:
            logger.info("Document Agent: no state found in storage. Returning an initiated state.")
            self._state = None
        return self._state
    ...
    async def _run(
        ...
        if state is None:
            state = State(mode=Mode(name=mode_name, status=ModeStatus.INITIATED))
...
#    metadata: dict[str, Any] = {},
# ) -> None:
#    self._state = self._load_state(context)
#
