...
import pytest
from assistant.agentic.detect_information_request_needs import detect_information_request_needs
...
class TestShareTools:
    ...
    def test_initialization(self, context):
        ...
        # Test ShareTools initialization
        share_tools = ShareTools(context)
        assert share_tools.context == context

        # Test coordinator tools
        coordinator_tool_functions = share_tools.conversationalist_tools()
        assert coordinator_tool_functions is not None

        # Verify Coordinator-specific functions are registered
        assert "update_brief" in coordinator_tool_functions.function_map
        assert "resolve_information_request" in coordinator_tool_functions.function_map

        # Verify Team-specific functions are NOT registered
        assert "create_information_request" not in coordinator_tool_functions.function_map
        assert "delete_information_request" not in coordinator_tool_functions.function_map

        # Test team tools
        team_tool_functions = share_tools.team_tools()
        assert team_tool_functions is not None

        # Verify Team-specific functions are registered
        assert "create_information_request" in team_tool_functions.function_map
        assert "delete_information_request" in team_tool_functions.function_map

        # Verify Coordinator-specific functions are NOT registered
        assert "update_brief" not in team_tool_functions.function_map
        assert "resolve_information_request" not in team_tool_functions.function_map

        # detect_information_request_needs is not exposed as a tool function anymore
        assert "detect_information_request_needs" not in team_tool_functions.function_map
    ...
    async def test_project_tools_with_config(self, context, monkeypatch):
        ...
        share_tools = ShareTools(context)
        coordinator_tools = share_tools.conversationalist_tools()

        # Make sure basic tools are available
        assert "update_brief" in coordinator_tools.function_map
        ...
        project_tool_functions = await check_tools_with_config(context, ConversationRole.COORDINATOR)

        # Verify basic tools are still available regardless of track_progress setting
        assert "update_brief" in project_tool_functions.function_map
        assert "resolve_information_request" in project_tool_functions.function_map

        # For team tools
        team_tool_functions = await check_tools_with_config(context, ConversationRole.TEAM)
        assert "create_information_request" in team_tool_functions.function_map
        assert "delete_information_request" in team_tool_functions.function_map
