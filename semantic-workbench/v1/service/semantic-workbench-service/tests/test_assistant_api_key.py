...
def test_cached_api_key_store():
    ...
    assert cached_store.get(key_name=key_name) == api_key
    ...
    reset_api_key = cached_store.reset(key_name=key_name)
    assert reset_api_key != updated_api_key
    assert cached_store.get(key_name=key_name) == reset_api_key

    # ensure that delete removes the item from the cache
    cached_store.delete(key_name=key_name)
    assert cached_store.get(key_name=key_name) is None

    # update the key again on the "backend" and ensure the item is not in the cache
    mock_store.override_api_key(key_name=key_name, api_key=updated_api_key)
    assert cached_store.get(key_name=key_name) is None
    ...
    cached_store.delete(key_name=key_name)
    mock_store.override_api_key(key_name=key_name, api_key=updated_api_key)
    assert cached_store.get(key_name=key_name) == updated_api_key
    ...
    assert cached_store.get(key_name=key_name) == another_updated_api_key

    # ensure that a different key has an isolated cache
    second_key_name = "second_key"
    second_api_key = cached_store.reset(key_name=second_key_name)
    assert cached_store.get(key_name=second_key_name) == second_api_key
